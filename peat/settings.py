...
class Configuration(SettingsManager):
    ...
    DEVICE_DIR: Path | Literal[""] = RUN_DIR / "devices"
    """
    ...
    ELASTIC_DIR: Path | Literal[""] = RUN_DIR / "elastic_data"
    """
    ...
    LOG_DIR: Path | Literal[""] = RUN_DIR / "logs"
    """
    ...
    META_DIR: Path | Literal[""] = RUN_DIR / "peat_metadata"
    """
...
class State(SettingsManager):
    ...
    elastic: Any = None
    """
    :class:`~peat.elastic.Elastic` instance used to interact with a Elasticsearch or
    OpenSearch database.

    Typed as :class:`~typing.Any` rather than :class:`~peat.elastic.Elastic` itself:
    this module cannot import other ``peat`` modules (see CONTRIBUTING.rst), and since
    :class:`SettingsManager` subclasses are real pydantic models, an unresolvable
    forward reference here would raise as soon as :data:`~peat.settings.state` is
    constructed, rather than being silently ignored.
